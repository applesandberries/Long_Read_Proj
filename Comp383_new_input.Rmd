...
ani_direct <- "/Users/leahboger/Desktop/Lparagasseri_ANI/"

# list all tsv files in the ANI directory
ani_files <- list.files(ani_direct, pattern = "\\.tsv$", full.names = TRUE)

columns_to_extract <- c(1, 3, 5)  # column numbers we want

# empty list to store column data from each file
data_list <- list()

# read data and extract specific column for each
for (file in ani_files) {
  data <- readr::read_tsv(file, col_names = FALSE)
  extracted_columns <- lapply(columns_to_extract, function(col) data[[col]])
  data_list[[length(data_list) + 1]] <- extracted_columns
}
...
#strain <- rep(paste0(label, basename(ani_files)), each = length(y) / length(ani_files))
#strain <- rep(ani_files, each = length(data_list[[1]]))
strain <- rep(paste0(label, seq_along(data_list)), each = length(data_list[[1]]))
```
