...
full_full <- readr::read_tsv("~/Downloads/full_full_ANI.txt", col_names = FALSE)
...
#kruskal.test(full_fullANI, rep(1, length(full_fullANI)))
...
```{r}
# Tukey's HSD
# TukeyHSD(aov(full_fullANI ~ 1), conf.level=0.95)
...
line_value <- subfull_ANI[1]  

# df for plotting
df_fullfull <- data.frame(ANI = fullfull_ANI, Type = "Full-to-Full")
line <- data.frame(ANI = line_value, Type = "Sub-to-Full")

# filter out 100% ANIs
df_combined <- rbind(df_fullfull, line)
